...
- **Full Bilingual Localization (Arabic/English)** — Complete i18n system in `client/src/lib/i18n.tsx` with 600+ translation keys covering all page namespaces. Every page uses `useI18n()` hook (`t()`, `lang`, `isRtl`). All hardcoded English strings replaced with `t()` calls. Full RTL support using logical CSS properties (`ms-`, `me-`, `ps-`, `pe-`, `start-`, `end-`). Cairo/Tajawal Arabic fonts loaded. RTL CSS appended to `client/src/index.css`. Sheet sides are direction-aware (`lang === "ar" ? "right" : "left"`). Currency values wrapped in `dir="ltr"` spans for correct display in RTL mode.
