...
export function FloatingAIChat() {
  ...
  return (
    <>
      ...
      {/* panel */}
      <div className={cn(
        "fixed bottom-24 right-6 z-50 w-96 max-w-[calc(100vw-24px)] rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col overflow-hidden transition-all duration-300",
        open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-4 pointer-events-none"
      )} style={{ height: 520, background: "#fff" }}>

        {/* header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800 shrink-0"
          style={{ background: `linear-gradient(135deg, ${BRAND}08, ${PURPLE}08)` }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-bold text-sm text-gray-900">Wealthly AI</p>
              <p className="text-[10px] text-gray-400">Your personal finance assistant</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            {messages.length > 0 && (
              <button onClick={clearChat} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors" title="Clear chat">
                <RotateCcw className="w-3.5 h-3.5" />
              </button>
            )}
            <button onClick={() => setOpen(false)} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
              ...
              <div className="space-y-1.5">
                {SUGGESTED_PROMPTS.map(p => (
                  <button key={p} onClick={() => handleSend(p)}
                    className="w-full text-left px-3 py-2 rounded-xl border border-gray-200 bg-white text-xs text-gray-700 hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 transition-all flex items-center justify-between group shadow-sm">
                    {p}
                    <ChevronRight className="w-3 h-3 text-gray-300 group-hover:text-blue-400 shrink-0" />
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="px-3 py-3 border-t border-gray-100 bg-white shrink-0">
          ...
          <p className="text-[10px] text-center text-gray-300 mt-1.5">Powered by GPT · Based on your live financial data</p>
        </div>
      </div>
    </>
  );
