...
export function FloatingAIChat() {
  ...
  const SUGGESTED_PROMPTS = [
    t("aiCoach.suggested1", { defaultValue: "How much did I spend this month?" }),
    t("aiCoach.suggested2", { defaultValue: "Am I on track with my savings goals?" }),
    t("aiCoach.suggested3", { defaultValue: "What's my biggest expense category?" }),
    t("aiCoach.suggested4", { defaultValue: "Should I pay off debt or invest?" }),
    t("aiCoach.suggested5", { defaultValue: "What's my current net worth?" }),
    t("aiCoach.suggested6", { defaultValue: "Calculate my Zakat for this year" }),
  ];
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "assistant", content: t("aiCoach.errorConnecting"), ts: new Date() }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className={cn(
          "fixed bottom-6 z-50 w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-blue-500/30 transition-all duration-200 hover:scale-105 active:scale-95",
      ...
      {/* panel */}
      <div className={cn(
        "fixed bottom-24 z-50 w-96 max-w-[calc(100vw-24px)] rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden transition-all duration-300 bg-white dark:bg-gray-900",
        isRtl ? "left-6" : "right-6",
        open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-4 pointer-events-none"
      )} style={{ height: 520 }} dir={isRtl ? "rtl" : "ltr"}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-blue-50/60 to-purple-50/60 dark:from-blue-950/30 dark:to-purple-950/30 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div className={isRtl ? "text-right" : "text-left"}>
              <p className="font-bold text-sm text-gray-900 dark:text-white">{t("common.appName")} AI</p>
              <p className="text-[10px] text-gray-400">{t("common.tagline")}</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            {messages.length > 0 && (
              <button onClick={clearChat} className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" title={t("common.refresh")}>
          ...
          {messages.length === 0 ? (
            <div className="space-y-3">
              <div className="flex items-start gap-2.5">
                ...
                <div className={cn("bg-white dark:bg-gray-800 rounded-2xl px-3.5 py-2.5 shadow-sm border border-gray-100 dark:border-gray-700 max-w-[85%]", isRtl ? "rounded-tr-sm" : "rounded-tl-sm")}>
                  <p className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed">
                    {t("aiCoach.greeting", { name: "" })}
                  </p>
                </div>
              </div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 px-1 pt-1">{t("dashboard.aiInsight")}:</p>
              <div className="space-y-1.5">
                {SUGGESTED_PROMPTS.map(p => (
                  <button key={p} onClick={() => handleSend(p)}
                    ...
                    {p}
                    <ChevronRight className={cn("w-3 h-3 text-gray-300 dark:text-gray-600 group-hover:text-blue-400 shrink-0", isRtl && "rotate-180")} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <>
              {messages.map((m, i) => (
                <div key={i} className={cn("flex gap-2.5", m.role === "user" ? (isRtl ? "flex-row" : "flex-row-reverse") : (isRtl ? "flex-row-reverse" : "flex-row"))}>
                  {m.role === "assistant" && (
                  ...
                  <div className={cn(
                    ...
                    m.role === "user"
                      ? cn("text-white", isRtl ? "rounded-tl-sm" : "rounded-tr-sm")
                      : cn("bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-800 dark:text-gray-200", isRtl ? "rounded-tr-sm" : "rounded-tl-sm")
                  )}
              ...
              {loading && (
                <div className={cn("flex gap-2.5", isRtl ? "flex-row-reverse" : "flex-row")}>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                    style={{ background: `linear-gradient(135deg, ${BRAND}, ${PURPLE})` }}>
                    <Sparkles className="w-3.5 h-3.5 text-white" />
                  </div>
                  <div className={cn("bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm", isRtl ? "rounded-tr-sm" : "rounded-tl-sm")}>
                    <TypingIndicator />
                  </div>
                </div>
              )}
        ...
        <div className="px-3 py-3 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shrink-0">
          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 focus-within:border-blue-400 dark:focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 dark:focus-within:ring-blue-900/30 transition-all">
            ...
            <button
              ...
              <Send className={cn("w-3.5 h-3.5 text-white", isRtl && "rotate-180")} />
            </button>
          </div>
          <p className="text-[10px] text-center text-gray-300 dark:text-gray-600 mt-1.5">{t("dashboard.fullAiReport")}</p>
        </div>
      </div>
