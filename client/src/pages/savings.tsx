  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-[#1a1a1a]" data-testid="text-page-title">Bank Savings</h2>
          <p className="text-[#666666] mt-1">Track monthly savings by bank.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666] mb-2">This Month</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]" data-testid="text-monthly-savings">{formatCurrency(totalMonthlySavings)}</h3>
              </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666] mb-2">% of Income</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]">{savingsPct.toFixed(1)}%</h3>
              </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666] mb-2">All-Time Savings</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]">{formatCurrency(totalAllTimeSavings)}</h3>
              </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl bg-white mb-6">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              <div>
                <label className="text-sm font-medium text-[#666] mb-1 block">Bank Name</label>
          ...
          <CardContent>
            <div className="space-y-4">
              {bankBreakdown.map((bank, i) => (
                <div key={i} className="flex justify-between items-center p-4 rounded-xl bg-[#f8f9fa]">
                  <div className="flex items-center gap-3">
                    <Building2 className="w-5 h-5 text-blue-600" />
                    <span className="font-medium">{bank.name}</span>
                  </div>
                  <span className="font-bold text-[#1a1a1a]">{formatCurrency(bank.amount)}</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <Card className="border-none shadow-sm rounded-2xl bg-white">
        ...
        <CardContent>
          {monthTransactions.length === 0 ? (
            <p className="text-center text-[#999] py-8">No savings recorded this month.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-[#666]">Bank Name</th>
                    <th className="text-right py-3 px-4 font-semibold text-[#666]">Saved Amount</th>
                ...
                <tbody>
                  {monthTransactions
                    ...
                    .map((t: any) => (
                      <tr key={t.id} className="border-b hover:bg-[#f8f9fa] transition-colors" data-testid={`row-savings-${t.id}`}>
                        <td className="py-3 px-4">{format(new Date(t.date), "MMM d, yyyy")}</td>
                        <td className="py-3 px-4 font-medium">{t.description}</td>
                        <td className="py-3 px-4 text-right font-bold text-blue-600">{formatCurrency(Number(t.amount))}</td>
