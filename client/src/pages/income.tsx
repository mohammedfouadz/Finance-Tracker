  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-[#1a1a1a]" data-testid="text-page-title">Income</h2>
          <p className="text-[#666666] mt-1">Track and manage your income sources.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666666] mb-2">Total Income ({selectedYear})</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]" data-testid="text-total-income">{formatCurrency(totalIncome)}</h3>
              </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666666] mb-2">This Month</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]" data-testid="text-monthly-income">{formatCurrency(currentMonthIncome)}</h3>
              </div>
        ...
        <Card className="border-none shadow-sm rounded-2xl bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-[#666666] mb-2">Monthly Average</p>
                <h3 className="text-2xl font-bold text-[#1a1a1a]">{formatCurrency(totalIncome / 12)}</h3>
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-3 px-4 font-semibold text-[#666]">Month</th>
                  <th className="text-right py-3 px-4 font-semibold text-[#666]">Total Income</th>
                </tr>
              </thead>
              <tbody>
                {MONTHS.map((month, i) => (
                  <tr key={i} className="border-b hover:bg-[#f8f9fa] transition-colors">
                    <td className="py-3 px-4 font-medium">{month}</td>
                    <td className="py-3 px-4 text-right font-bold text-[#1a1a1a]">{formatCurrency(monthlyTotals[i])}</td>
                  </tr>
                ))}
                <tr className="bg-primary/5 font-bold">
                  <td className="py-3 px-4">Total</td>
                  <td className="py-3 px-4 text-right text-primary">{formatCurrency(totalIncome)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <Card className="border-none shadow-sm rounded-2xl bg-white">
        ...
        <CardContent>
          {incomeTransactions.length === 0 ? (
            <p className="text-center text-[#999] py-8">No income entries yet. Click "Add Income" to get started.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {incomeTransactions
                    ...
                    .map((t: any) => {
                      ...
                      return (
                        <tr key={t.id} className="border-b hover:bg-[#f8f9fa] transition-colors" data-testid={`row-income-${t.id}`}>
                          ...
                          <td className="py-3 px-4 text-right font-bold text-green-600">{formatCurrency(Number(t.amount))}</td>
